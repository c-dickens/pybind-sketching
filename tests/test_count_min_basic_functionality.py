...
def test_single_string_updates(fixed_sketch_setup):
    """
    Tests updates are performed as expected under the hash functions.
    :return:
    :rtype:

    Nb. for this seed we know from inspection in the main.cpp that str(1) is hashed to the buckets [8, 2, 7].
    No extra weight is added so each of these sketch entries should be equal to 1
    """
    item = str(1)
    c = CountMinSketch(*fixed_sketch_setup)
    assert c.get_estimate(item) == 0
    c.update(item)
    sk = c.get_table()
    print(np.array(sk))
    assert sk[0][8] == 1
    ...
    assert sk[2][7] == 10
    # no other items inserted so estimation should be exact.
    assert c.get_estimate(item) == 10
    c.update(item, -10) # Remove all weight so the buckets should now have a zero.
    sk = c.get_table()
    assert sk[0][8] == 0
    assert sk[1][2] == 0
    assert sk[2][7] == 0
